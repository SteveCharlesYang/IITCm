...
function createDefaultBaseMapLayers() {
  ...
  baseLayers['Google 默认Ingress地图'] = new L.Google('ROADMAP',{maxZoom:21, mapOptions:ingressGMapOptions});
  baseLayers['Google 道路地图'] = new L.Google('ROADMAP',{maxZoom:21});
  baseLayers['Google 卫星地图'] = new L.Google('SATELLITE',{maxZoom:21});
  baseLayers['Google 混合地图'] = new L.Google('HYBRID',{maxZoom:21});
  baseLayers['Google 地形地图'] = new L.Google('TERRAIN',{maxZoom:15});
...
//    zoomAnimation: false,
  ...
  for(var i = 0; i <= 8; i++) {
    ...
    var t = (i === 0 ? '无人占领/占位' : '等级为 ' + i) + ' 的Portal';
...
window.setupPlayerStat = function() {
  ...
  $('#playerstat').html(''
    ...
    + '<div id="stats">'
    + '<sup>XM: '+xmRatio+'%</sup>'
    + '<sub>' + (nextLvlAp > 0 ? '等级: '+lvlApProg+'%' : '满级') + '</sub>'
    + '</div>'
    + '</h2>'
  );
}
...
// globalBounds: (optional) if true, always use global latLng bounds
  {channel:'all', name:'所有讯息', inputPrompt: '广播通信:', inputColor:'#f66'},
  {channel:'faction', name:'阵营讯息', inputPrompt: '阵营通信:'},
  {channel:'alerts', name:'警报', inputPrompt: '告诉Jarvis:', inputColor: '#666', globalBounds: true, sendMessage: function() {
...
window.renderPortalDetails = function(guid) {
  ...
  if(!window.portals[guid]) {
    ...
    if(isSmartphone()) {
      $('.fullimg').remove();
      $('#mobileinfo').html('<div style="text-align: center"><b>点击此处查看信息界面</b></div>');
    }
    return;
  }
...
//TODO? other status details...
  ...
  if (typeof android !== 'undefined' && android && android.intentPosLink) {
    ...
    var shareLink = $('<div>').html( $('<a>').attr({onclick:posOnClick}).text('分享') ).html();
...
window.regionScoreboard = function() {
  ...
  var dlg = dialog({title:'地区记分',html:'加载地区记分...',width:450,minHeight:320});
...
function regionScoreboardSuccess(data,dlg,logscale) {
  ...
  dlg.html('<div class="cellscore">'
         +'<b>'+data.result.regionName+' 地区的区域记分</b>'
